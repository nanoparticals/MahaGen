...
class _PredictorWrapper:
    def __init__(self, model_path: str, featurizer: str = "morgan2048"):
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "请检查环境/版本（例如 chemprop==1.6.1）以及依赖是否完整。"
            ) from e

        self._cp_make_predictions = make_predictions

        # 读取训练时的 args.json，镜像 features 配置
        feats_gen: List[str] = []
        ...
        if feats_gen:
            ...
            # chemprop 1.6.1 规则：rdkit_2d_normalized 必须带 --no_features_scaling
        ...
        # 准备临时 test/preds 文件（chemprop 需要）
    ...
    def predict(self, smiles: List[str]) -> np.ndarray:
        # chemprop 需要二维 smiles 列表 [[s],[s],...]
        smi2d = [[s] for s in smiles]
        preds = self._cp_make_predictions(args=self._cp_args, smiles=smi2d)
