...
# -*- coding: utf-8 -*-
"""
Li 电解液方向：给 CSV 增加 RDKit TPSA，并画 TPSA parity 图 (论文发表级样式)。

用法示例：
python add_rdkit_tpsa_styled.py \
  --csv  ./data/test_full.csv \
  --out  ./data/test_full_with_tpsa.csv \
  --plot ./plots/tpsa_parity.png \
  --smiles-col smiles \
  --phys-col "TPSA" \
  --quiet-rdkit
"""
...
# 2. RDKit 计算功能
# ==========================================
def compute_rdkit_tpsa(smiles: str) -> float:
    """Return RDKit TPSA; NaN if invalid."""
...
def plot_parity_styled(x, y, xlabel, ylabel, out_path):
    ...
    # 4. 标签设置 (增加 TPSA 单位)
    # 如果列名里没写单位，自动加上 Angstrom^2
    unit_suffix = " ($\mathring{A}^2$)" if "A^2" not in xlabel and "TPSA" in xlabel.upper() else ""
    
    g.ax_joint.set_xlabel(f"{xlabel}{unit_suffix}")
    g.ax_joint.set_ylabel(f"{ylabel}{unit_suffix}")
    g.ax_joint.set_xlim(lo, hi)
    g.ax_joint.set_ylim(lo, hi)

    # 5. 统计框 (包含 N, MAE, RMSE, R2)
    stats_text = (f"N = {n_count}\n"
                  f"MAE: {mae:.3f} $\\mathring{{A}}^2$\n"
                  f"RMSE: {rmse:.3f} $\\mathring{{A}}^2$\n"
                  f"R$^2$: {r2:.3f}")
...
def main():
    ...
    ap.add_argument("--phys-col", default="pred_TPSA", help="物理尺度值列名 (y轴)")
    ap.add_argument("--rdkit-col", default="TPSA_rdkit", help="RDKit 计算值列名 (x轴)")
    ...
    # 如果物理列不存在，可能是用户想算 TPSA 但没有对比值，我们先算算看
    has_phys = args.phys_col in df.columns
    if not has_phys:
        print(f"Warning: 物理列 '{args.phys_col}' 不存在，将只计算 RDKit 值，不画图。")

    print(f"正在计算 RDKit TPSA (Total: {len(df)})...")
    # 计算 rdkit_TPSA
    rdkit_vals = [compute_rdkit_tpsa(str(s)) for s in df[args.smiles_col]]
    ...
    if has_phys:
        ...
        # 将列名作为轴标签 (去除下划线可能好看点，但为了准确性还是保留)
        xlabel = "RDKit TPSA"
        ylabel = args.phys_col
