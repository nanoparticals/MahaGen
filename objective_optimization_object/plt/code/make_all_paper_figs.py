...
# -*- coding: utf-8 -*-
"""
make_nature_figs_final.py
Fixes:
1. Defined missing 'murcko_scaffold' function.
2. Added .copy() to prevent SettingWithCopyWarning.
3. Strict filtering (Sim < 0.999) for plotting.
"""
...
def get_mol_weight_logp(smi):
    m = Chem.MolFromSmiles(smi)
    return (Descriptors.MolWt(m), Descriptors.MolLogP(m)) if m else (np.nan, np.nan)
...
def plot_fig2_reward_vs_novelty_hex(df, out_dir):
    """
    Fig 2 V8 Fix: 
    - Aggressively shrink right margin to 0.80 to ensure Color Bar Label fits.
    - Move Color Bar to 0.83.
    """
    ...
    # 6. Add Color Bar (MOVED LEFT AGAIN)
    # New Position: [0.83, 0.15, 0.02, 0.35]
    # This leaves space from 0.85 to 1.00 for the text label.
    ...
    # CRITICAL FIX: Right margin set to 0.80
...
def main():
    ...
    ap.add_argument("--out_dir", default="nature_figs_final")
    args = ap.parse_args()
    ...
    # 1. Unique (FIX 2: Added .copy() to avoid SettingWithCopyWarning)
    ...
    # Now this assignment is safe
    gen_unique["is_in_train"] = gen_unique["smiles_canon"].isin(train_set)
    novelty = 1.0 - gen_unique["is_in_train"].mean()
    
    # This calls the helper function 'murcko_scaffold' which is now defined above
    ...
    # 3. STRICT FILTER: Remove anything with Sim >= 0.999
    gen_novel_strict = gen_unique[gen_unique["max_sim_train"] < 0.999].copy()
    
    print(f"Total Unique: {len(gen_unique)}, Strictly Novel (Sim<0.999): {len(gen_novel_strict)}")

    # Train internal sim sample (for Fig 1)
    import random
    ...
    # Tradeoff Data (Using Strictly Novel)
    ...
    print("Generating figures (Strictly Novel)...")
    ...
    target_x = "EG_phys_m_s"  # 你的CSV列名
    target_y = "D_phys_km_s"   # 你的CSV列名
